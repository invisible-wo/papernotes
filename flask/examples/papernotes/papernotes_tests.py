...
"""
    ...
    Tests the PaperNotes application.
...
class PaperNotesTestCase(unittest.TestCase):

    def setUp(self):
        """Before each test, set up a blank database"""
        self.db_fd, papernotes.app.config['DATABASE'] = tempfile.mkstemp()
        self.app = papernotes.app.test_client()
        with papernotes.app.app_context():
            papernotes.init_db()

    def tearDown(self):
        """Get rid of the database again after each test."""
        os.close(self.db_fd)
        os.unlink(papernotes.app.config['DATABASE'])
